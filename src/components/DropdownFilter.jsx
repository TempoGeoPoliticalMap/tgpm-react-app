  ...
  return (
    <div className="relative inline-flex align-top">
      ...
      <Transition
        show={dropdownOpen}
        tag="div"
        className={`origin-top-right z-10 absolute top-full min-w-max bg-white border border-slate-200 pt-1.5 rounded shadow-lg overflow-hidden mt-1 ${
        ...
        <div ref={dropdown}>
          ...
          <ul className="mb-4">
            {types.map(type => {
              return (
                <li key={type} className="py-1 px-3">
                  <label className="flex items-center">
                    <input type="checkbox" className="form-checkbox" />
                    <span className="text-sm font-medium ml-2">{TYPES[type].NAME}</span>
                  </label>
                </li>
              );
            })}
